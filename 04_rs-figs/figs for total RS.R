...
fem.prop.rs <- ggplot(fem, aes(x=season.prop.ep, y=tot_fledge, group=ep.yes)) +
  ...
  geom_smooth(method=lm, se=F, linetype="dashed", color="#CC00FF")+
  theme_light() 
...
fem.num.rs <- ggplot(fem, aes(x=num.epo, y=tot_fledge)) +
  ...
  geom_smooth(method=lm, se=F, color="#CC00FF") +
  ylab(NULL) +
...
male.ep.rs <- ggplot(male, aes(x=ep.yes.factor, y=tot.chick)) +
  geom_boxplot(fill="#00B0F0") + ylim(0,16) +
  geom_point(alpha=0.5,
             position=position_jitter(width=0.2, height=0))+
...
male.num.rs <- ggplot(male, aes(x=ep.chick, y=tot.chick)) +
  ...
  geom_smooth(method=lm, se=F, color="#00B0F0") +
...
male.prop.rs <- ggplot(male, aes(x=prop.sired.epo, y=tot.chick)) +
  ...
  geom_smooth(method=lm, se=F, linetype="solid", color="#00B0F0") +
...
ggsave("RS patterns female and male combined_2025-03-13.png", h=11, w=8)
