...
fem.numEPO.adj1 <- glm.nb(num.epo ~ tail_scaled + socM_tail_scaled + 
                            ...
                            ci_1_julian_scaled +
                            offset(log(tot.chick)), data=fem)

summary(fem.numEPO.adj1)
...
pred.fem.numEPO.throat <-  
  data.frame(tail_scaled = rep(mean(fem$tail_scaled, na.rm=T), fem.throat.n),
...
# It doesn't make sense to add an offset for this male model, because males are
# not limited in the number of EPO they can sire. Any chicks sampled outside
# the male's social nest(s) could possibly be EPO of any sampled males. 
...
save(numEPO.focal.breast, numEPO.focal.tail, numEPO.focal.throat, 
     ...
     file="output-files/numEPO combined plots_2025-10-02_bluetan.Rdata")
